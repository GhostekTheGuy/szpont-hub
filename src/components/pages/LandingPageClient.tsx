...
export default function LandingPageClient({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  useEffect(() => {
    // Prefetch destinations so navigation from the loading overlay is instant
    router.prefetch(isLoggedIn ? '/dashboard' : '/login?mode=register');
  ...
  const navigateTo = useCallback((href: string) => {
    setNavigating(true);
    // Allow the loading overlay to paint before kicking off the route change.
  ...
  return (
    ...
    {/* Loading overlay — Matrix-style background with glitching "szpont" */}
    {navigating && (
      <div className="fixed inset-0 z-[200] bg-black animate-fade-in">
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-6 px-6">
          <div className="font-mono text-5xl sm:text-7xl md:text-8xl font-extrabold tracking-tight text-white drop-shadow-[0_0_30px_rgba(124,58,237,0.7)] animate-pulse">
            <span className="text-primary">$zpont</span>
          </div>
          <div className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.3em] text-white/70">
